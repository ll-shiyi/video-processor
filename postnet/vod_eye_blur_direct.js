// vod_eye_blur_direct.js
// 使用processor_direct.js的直接视频处理版本
// 完全参照Pose.jsx逻辑，不使用任何优化
...
// ---------- 主函数 ----------
async function maskEyesWithPoseNetDirect(opts) {
  const startTime = Date.now();
  const startTimeISO = new Date().toISOString();
  console.log('[START] VOD视频遮挡处理启动 (直接处理版本，完全参照Pose.jsx)');
  ...
  const {
    ...
    // 保存无脸帧参数
    saveNoFaceFrames = false,
    noFaceDir = 'no_face_frames',
  } = opts;
  ...
  const tmpFile = path.join(os.tmpdir(), `vod-privacy-guard-direct-${Date.now()}-${process.pid}.mp4`);
  ...
  try {
    ...
    // 4) 处理器：使用processor_direct.js，完全参照Pose.jsx逻辑
    console.log('[PROCESSOR] 启动 processor_direct…');
    const processorPath = path.join(__dirname, 'processor_direct.js');
    const processorArgs = [
      processorPath,
      '--width', String(SRC_W),
      '--height', String(SRC_H),
      ...
      '--nmsRadius', String(nmsRadius),
      '--minPoseConfidence', String(minPoseConfidence),
      ...
      // PoseNet模型参数（与前端Pose.jsx完全一致）
      '--quantBytes', String(quantBytes),
      '--multiplier', String(multiplier),
      '--outputStride', String(outputStride),
      '--inputResolution', String(inputResolution),

      // 保存无脸帧参数
      '--saveNoFaceFrames', String(saveNoFaceFrames),
      '--noFaceDir', String(noFaceDir),
    ...
    // 管线连接
    console.log('[PIPELINE] 连接 ffmpeg-in → processor_direct');
    ffmpegIn.stdout.pipe(processor.stdin, { highWaterMark: 1024 * 1024 });

    console.log('[PIPELINE] 连接 processor_direct → ffmpeg-out');
    ...
    const uploadResult = await uploader.uploadToVod(tmpFile, vodClient, {
      ...
      fileName: `processed_direct_${videoId}_${Date.now()}.mp4`,
      fileSize: stat.size,
      cateId: 0,
      tags: 'privacy-protected,eye-blur,direct-processing',
      // 分片上传配置
    ...
    const successResult = {
      ...
      fps: FPS,
      sar: origSAR,
      processingType: 'direct', // 标识这是直接处理版本
...
module.exports = { maskEyesWithPoseNetDirect };
